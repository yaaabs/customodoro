...
(function () {
  ...
  function displayImagePreview(dataUrl, file) {
    ...
    showToast(
      'Image uploaded successfully! Click "Preview" to see it as a theme.',
    );
  }
  ...
  function saveCustomTheme(dataUrl) {
    try {
      ...
      if (
        ...
        showToast(
          "Image is too large to save. Please try a smaller image or clear some browser storage.",
        );
  ...
  function setupColorThemeListeners() {
    ...
    if (customColorInput) {
      ...
      customColorInput.addEventListener("focus", (e) => {
        ...
        if (colorPreviewIndicator) {
          colorPreviewIndicator.style.backgroundColor =
            e.target.value || selectedColor;
          colorPreviewIndicator.style.setProperty(
            "--current-color",
            e.target.value || selectedColor,
          );
        }
