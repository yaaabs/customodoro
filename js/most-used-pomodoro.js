...
(function () {
  ...
  function computeAndRender() {
    ...
    const classicPct =
      total === 0 ? 0 : Math.round((classic / total) * 10000) / 100; // 2 decimal precision
    const reversePct =
      total === 0 ? 0 : Math.round((reverse / total) * 10000) / 100;
    ...
    if (card) {
      card.setAttribute(
        "aria-label",
        `Most used pomodoro: classic ${classicPct} percent, reverse ${reversePct} percent`,
      );
    }
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    if (classicNode || reverseNode) {
      ...
      if (classicNode)
        obs.observe(classicNode, {
          childList: true,
          characterData: true,
          subtree: true,
        });
      if (reverseNode)
        obs.observe(reverseNode, {
          childList: true,
          characterData: true,
          subtree: true,
        });
