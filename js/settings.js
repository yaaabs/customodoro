...
(function () {
  ...
  function openSettings() {
    ...
    setTimeout(function () {
      if (
        window.lockedInMode &&
        typeof window.lockedInMode.setup === "function"
      ) {
      ...
      if (
        window.colorTheme &&
        typeof window.colorTheme.updateColorThemeVisibility === "function"
      ) {
      ...
      if (
        isReversePage &&
        window.bgmPlayer &&
        !window.bgmPlayer.isInitialized
      ) {
  ...
  function closeSettings() {
    ...
    try {
      if (
        window.bgmPlayer &&
        typeof window.bgmPlayer.saveState === "function"
      ) {
  ...
  function resetSettings() {
    if (
      ...
      const pomodoroVolumePercentage = document.getElementById(
        "pomodoro-volume-percentage",
      );
      const breakVolumeSlider = document.getElementById("break-volume-slider");
      const breakVolumePercentage = document.getElementById(
        "break-volume-percentage",
      );
      const soundEffectsToggle = document.getElementById(
        "sound-effects-toggle",
      );
      const alarmToggle = document.getElementById("alarm-toggle");
      const pomodoroSoundSelector = document.getElementById(
        "pomodoro-sound-selector",
      );
      const breakSoundSelector = document.getElementById(
        "break-sound-selector",
      );

      if (pomodoroVolumeSlider) pomodoroVolumeSlider.value = 60;
      if (pomodoroVolumePercentage)
        pomodoroVolumePercentage.textContent = "60%";
      ...
      const timerSoundSelector = document.getElementById(
        "timer-sound-selector",
      );
      const timerSoundVolumeSlider = document.getElementById(
        "timer-sound-volume-slider",
      );
      const timerSoundVolumePercentage = document.getElementById(
        "timer-sound-volume-percentage",
      );
      ...
      if (timerSoundVolumePercentage)
        timerSoundVolumePercentage.textContent = "60%";

      // Update BGM settings visually
      const bgmToggle = document.getElementById("bgm-toggle");
      const playlistSelector = document.getElementById("playlist-selector");
      const bgmVolumeSlider = document.getElementById("bgm-volume-slider");
      const bgmVolumePercentage = document.getElementById(
      ...
      const lockedInModeToggle = document.getElementById(
      ...
      if (
        ...
        typeof window.stopTimerSound === "function"
      ) {
        window.stopTimerSound();
      }

      // Reset tracker design selector visually and apply the design
      const trackerDesignRadios = document.querySelectorAll(
        'input[name="tracker-design"]',
      );
  ...
  function applyTheme(themeName) {
    ...
    if (themeName === "custom") {
      ...
      if (customTheme) {
        ...
        if (toast) {
          toast.textContent =
            "No custom theme found. Please upload an image in settings.";
      ...
      preloadImg.onerror = function () {
        ...
        if (toast) {
          toast.textContent =
            "Failed to load theme image. Using light theme instead.";
          toast.classList.add("show");
          setTimeout(() => toast.classList.remove("show"), 3000);
        }
        // Update the selector to match actual theme
  ...
  function saveLockedInModeSettings() {
    ...
    if (lockedInModeToggle) {
      ...
      if (
        window.lockedInMode &&
        typeof window.lockedInMode.setEnabled === "function"
      ) {
  ...
  function saveBurnupTrackerSettings() {
    const burnupTrackerToggle = document.getElementById(
    ...
    if (burnupTrackerToggle) {
      ...
      if (
        window.setBurnupTrackerEnabled &&
        typeof window.setBurnupTrackerEnabled === "function"
      ) {
  ...
  function forceTimerUpdate() {
    if (isReversePage) {
      ...
      document.querySelector(".max-time").textContent = `Max Time: ${Math.floor(
        newMaxTime / 60,
      )
        .toString()
        .padStart(2, "0")}:${(newMaxTime % 60).toString().padStart(2, "0")}:00`;
  ...
  function applyReverseSettings() {
    ...
    if (typeof window.calculateBreakTime === "function") {
      window.calculateBreakTime = function (workedSeconds) {
        const breakLogicMode =
          localStorage.getItem("breakLogicMode") || "default";
        const maxWorkMinutes =
          parseInt(localStorage.getItem("reverseMaxTime")) || 60;
  ...
  function saveReverseSettings() {
    const breakLogicMode = document.getElementById(
      "break-logic-mode-selector",
    ).value;
  ...
  function loadReverseSettings() {
    const breakLogicModeSelector = document.getElementById(
      "break-logic-mode-selector",
    );
  ...
  function loadSoundSettings() {
    ...
    if (pomodoroVolumePercentage) {
      pomodoroVolumePercentage.textContent =
        (localStorage.getItem("pomodoroVolume") || 60) + "%";
    }
    if (breakVolumeSlider) {
      breakVolumeSlider.value = localStorage.getItem("breakVolume") || 60;
    }
    if (breakVolumePercentage) {
      breakVolumePercentage.textContent =
        (localStorage.getItem("breakVolume") || 60) + "%";
    }
    ...
    if (pomodoroSoundSelector) {
      const savedPomodoroSound =
        localStorage.getItem("pomodoroSound") || "bell.mp3";
    ...
    if (timerSoundVolumePercentage) {
      timerSoundVolumePercentage.textContent =
        (localStorage.getItem("timerSoundVolume") || 60) + "%";
    }
  ...
  function updateSoundsDirectly() {
    if (typeof window.sounds !== "undefined") {
      ...
      const pomodoroVolume =
        parseInt(localStorage.getItem("pomodoroVolume") || 60) / 100;
      const breakVolume =
        parseInt(localStorage.getItem("breakVolume") || 60) / 100;
      ...
      const selectedBreakSound =
        localStorage.getItem("breakSound") || "bell.mp3";
      ...
      if (window.sounds.click)
        window.sounds.click.volume = soundsEnabled ? pomodoroVolume * 0.5 : 0;
      if (window.sounds.start)
        window.sounds.start.volume = soundsEnabled ? pomodoroVolume * 0.6 : 0;
      if (window.sounds.pause)
        window.sounds.pause.volume = soundsEnabled ? pomodoroVolume * 0.5 : 0;

      // Update alarm volumes
      if (window.sounds.pomodoroComplete)
        window.sounds.pomodoroComplete.volume = alarmEnabled
          ? pomodoroVolume
          : 0;
      if (window.sounds.breakComplete)
        window.sounds.breakComplete.volume = alarmEnabled ? breakVolume : 0;
  ...
  function testSound(type) {
    ...
    if (type === "pomodoroComplete") {
      ...
      const selectedSound = pomodoroSoundSelector
        ? pomodoroSoundSelector.value
        : "alarm.mp3";
      ...
      const volume = pomodoroVolumeSlider
        ? parseInt(pomodoroVolumeSlider.value) / 100
        : 0.6;
      ...
      const selectedSound = breakSoundSelector
        ? breakSoundSelector.value
        : "bell.mp3";

      // Apply the current volume settings
      const breakVolumeSlider = document.getElementById("break-volume-slider");
      const volume = breakVolumeSlider
        ? parseInt(breakVolumeSlider.value) / 100
        : 0.6;
      ...
      const selectedSound = timerSoundSelector
        ? timerSoundSelector.value
        : "none";
      ...
      const volume = timerSoundVolumeSlider
        ? parseInt(timerSoundVolumeSlider.value) / 100
        : 0.6;
...
*/
  ...
  function setupTimeControls() {
    ...
    const controls = [
      {
        minus: "pomodoro-minus-btn",
        plus: "pomodoro-plus-btn",
        input: "pomodoro-time",
        min: 1,
        max: 999,
      },
      {
        minus: "short-break-minus-btn",
        plus: "short-break-plus-btn",
        input: "short-break-time",
        min: 1,
        max: 999,
      },
      {
        minus: "long-break-minus-btn",
        plus: "long-break-plus-btn",
        input: "long-break-time",
        min: 5,
        max: 999,
      },
      {
        minus: "sessions-minus-btn",
        plus: "sessions-plus-btn",
        input: "sessions-count",
        min: 1,
        max: 999,
      },
      {
        minus: "max-time-minus-btn",
        plus: "max-time-plus-btn",
        input: "max-time",
        min: 15,
        max: 360,
      },
      ...
      {
        minus: "break1-minus-btn",
        plus: "break1-plus-btn",
        input: "break1-time",
        min: 1,
        max: 360,
      },
      {
        minus: "break2-minus-btn",
        plus: "break2-plus-btn",
        input: "break2-time",
        min: 2,
        max: 360,
      },
      {
        minus: "break3-minus-btn",
        plus: "break3-plus-btn",
        input: "break3-time",
        min: 5,
        max: 360,
      },
      {
        minus: "break4-minus-btn",
        plus: "break4-plus-btn",
        input: "break4-time",
        min: 10,
        max: 360,
      },
      {
        minus: "break5-minus-btn",
        plus: "break5-plus-btn",
        input: "break5-time",
        min: 15,
        max: 360,
      },
  ...
  function updateBreakTierLabels() {
    ...
    if (breakLogicMode === "dynamic") {
      ...
      if (labels[1])
        labels[1].textContent = `Break Time (1-${tier1} mins work)`;
      if (labels[2])
        labels[2].textContent = `Break Time (${tier1 + 1}-${tier2} mins work)`;
      if (labels[3])
        labels[3].textContent = `Break Time (${tier2 + 1}-${tier3} mins work)`;
      if (labels[4])
        labels[4].textContent = `Break Time (${tier3 + 1}-${tier4} mins work)`;
      if (labels[5])
        labels[5].textContent = `Break Time (${tier4 + 1}-${maxTime} mins work)`;
    } else {
      // default
  ...
  function initializeTrackerDesignSelector() {
    ...
    const savedDesign =
      localStorage.getItem("burnupTrackerDesign") || "match-theme";
  ...
  function applyTrackerDesign(designType, trackerElement = null) {
    const burnupTrackers = trackerElement
      ? [trackerElement]
      : document.querySelectorAll(".burnup-tracker");
