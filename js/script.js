...
let isBurnupTrackerEnabled =
  localStorage.getItem("burnupTrackerEnabled") !== "false"; // Default to true
...
function initializeSoundSettings() {
  ...
  const pomodoroVolume = localStorage.getItem("pomodoroVolume")
    ? parseInt(localStorage.getItem("pomodoroVolume")) / 100
    : 0.6;
  const breakVolume = localStorage.getItem("breakVolume")
    ? parseInt(localStorage.getItem("breakVolume")) / 100
    : 0.6;
  ...
  const selectedPomodoroSound =
    localStorage.getItem("pomodoroSound") || "alarm.mp3";
  ...
  try {
    ...
    if (soundName === "click") {
      ...
      const selectedAlarmSound =
        localStorage.getItem("alarmSound") || "alarm.mp3";
...
function initializeTimerSoundSettings() {
  ...
  const volume = localStorage.getItem("timerSoundVolume")
    ? parseInt(localStorage.getItem("timerSoundVolume")) / 100
    : 0.6;
...
if (taskDescInput) {
  ...
  taskDescInput.addEventListener("input", () => {
    hasUnsavedTasks =
      taskInput.value.trim().length > 0 ||
      taskDescInput.value.trim().length > 0;
  });
}
...
function updateBurnupTracker() {
  ...
  if (isRunning) {
    ...
    const progressPercent =
      initialSeconds > 0 ? (elapsedSeconds / initialSeconds) * 100 : 0;
...
function toggleTimer() {
  if (!isRunning) {
    ...
    if (
      window.lockedInMode &&
      typeof window.lockedInMode.isEnabled === "function" &&
...
function resetTimer() {
  ...
  if (currentMode === "pomodoro") {
    ...
    pomodoroTime =
      parseInt(localStorage.getItem("pomodoroTime")) || pomodoroTime;
    currentSeconds = pomodoroTime * 60;
  } else if (currentMode === "shortBreak") {
    shortBreakTime =
      parseInt(localStorage.getItem("shortBreakTime")) || shortBreakTime;
    currentSeconds = shortBreakTime * 60;
  } else {
    longBreakTime =
      parseInt(localStorage.getItem("longBreakTime")) || longBreakTime;
...
function switchMode(mode, autoStart = false) {
  ...
  if (!autoStart && isRunning) {
    const confirmed = confirm(
      "Timer is still running. Are you sure you want to switch modes? This will reset your current progress.",
    );
    if (!confirmed) return;
  }
  ...
  if (taskList.children.length > 0 && !autoStart && !isTimerPhaseSwitch) {
    const confirmed = confirm(
      "Switching modes will delete all your tasks. Do you want to continue?",
    );
    if (!confirmed) return;
  }
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (switchBtn) {
    switchBtn.addEventListener("click", async (e) => {
      ...
      const targetMode = switchBtn.textContent.includes("Reverse")
        ? "Reverse Pomodoro"
        : "Classic Pomodoro";
      ...
      if (window.taskRetentionManager && taskList.children.length > 0) {
        ...
        await window.taskRetentionManager.handleModeSwitch(
          targetUrl,
          targetMode,
        );
      } else {
        ...
        if (taskList.children.length > 0) {
          const confirmed = confirm(
            "Switching to a different timer type will delete all your tasks. Do you want to continue?",
          );
          if (!confirmed) {
            return;
          }
...
const motivationalAlertOverlay = document.getElementById(
  "motivational-alert-overlay",
);
const motivationalAlertMessage = document.getElementById(
  "motivational-alert-message",
);
...
function hideMotivationalAlert() {
  if (motivationalAlertOverlay)
    motivationalAlertOverlay.classList.remove("show");
}
...
function addDragAndDropListeners(taskItem) {
  ...
  taskItem.addEventListener("dragend", function () {
    ...
    document
      .querySelectorAll(".task-item.drop-above, .task-item.drop-below")
      .forEach((el) => {
...
function updateUnfinishedTasks() {
  const unfinishedTasks = Array.from(taskList.children).filter(
...
function parseDataKey(dateStrWithSuffix) {
  const [year, month, day] = dateStrWithSuffix
    .split("-")
...
function getColor(minutes, maxMinutes, emptyColor = "#ebedf0") {
  ...
  const q2 = Math.max(1, Math.ceil(maxMinutes * 0.5)); // 50% threshold (15 contributions)
...
function getMonthLabels(dates) {
  ...
  ranges.forEach((range) => {
    ...
    labels.push({
      month: new Date(2020, range.month, 1).toLocaleString("default", {
        month: "short",
      }),
      x,
    });
...
function renderContributionGraph() {
  ...
  if (selectedRange === "all") {
    ...
    const monthIdx = parseInt(parts[2], 10) - 1; // 0-based month
  ...
  for (let i = 0; i < 7; i++) {
    if (
      dayLabelsAccurate[i] === "Mon" ||
      dayLabelsAccurate[i] === "Wed" ||
      dayLabelsAccurate[i] === "Fri"
    ) {
      dayLabelIndexes.push(i);
    }
  }

  // Sizing - fixed cell size with horizontal scrollbar for overflow (keeps consistent cell sizing)
  const cellSize = 12,
    cellGap = 4;
  const leftPad = 36,
    topPad = 22,
    bottomPad = 22;
  // total width based on weeks (one column per week)
  const width = leftPad + weeks.length * (cellSize + cellGap) + 8;
  // ensure a sensible minimum width to avoid cramped layout
  ...
  if (
    ...
    body.classList.contains("theme-yourname") ||
    body.classList.contains("theme-rain")
  ) {
  ...
  const svgParts = [
    `<svg width="${width}" height="${height}" style="font-family:sans-serif;background:${bgColor};min-width:${minGraphWidth}px;display:block;">`,
  ];
  ...
  monthLabels.forEach((label) => {
    ...
    svgParts.push(
      `<text x="${x}" y="15" fill="${labelColor}" font-size="11" font-weight="500" text-anchor="middle">${label.month}</text>`,
    );
  });
  ...
  dayLabelIndexes.forEach((idx) => {
    ...
    svgParts.push(
      `<text x="4" y="${y}" fill="${labelColor}" font-size="11" font-weight="500" text-anchor="start">${dayLabelsAccurate[idx]}</text>`,
    );
  });
  ...
  weeks.forEach((week, x) => {
    week.forEach((date, y) => {
      ...
      const colorPoints =
        minutes > 0 ? Math.max(1, Math.floor(minutes / 5)) : 0;
      ...
      svgParts.push(
        `<rect x="${cellX}" y="${cellY}" width="${cellSize}" height="${cellSize}" rx="3" fill="${color}" stroke="${cellBorder}" stroke-width="1" data-date="${key}" data-classic="${dayStats.classic}" data-reverse="${dayStats.reverse}" data-break="${dayStats.break}" data-minutes="${minutes}" data-points="${points}" style="cursor:pointer;"/>`,
      );
    });
  });
...
(This Productivity Graph is inspired by GitHub's contribution calendar.)</div><br>
    ...
    const scrollThumbHover =
      mode === "dark" ? "rgba(255,255,255,0.24)" : "rgba(0,0,0,0.22)";
    style.textContent = `
    ...
    const newGraphContainer = container.querySelector(
      ".contribution-graph-scroll",
    );

    // Setup custom scrollbar (thumb under heatmap) and sync with scroll position
    const scrollTrack = container.querySelector(
      ".contribution-custom-scrollbar",
    );
    const scrollThumb = scrollTrack
      ? scrollTrack.querySelector(".contribution-custom-scrollbar-thumb")
      : null;
    ...
    function updateCustomScrollbar() {
      ...
      if (total <= visible) {
        scrollTrack.setAttribute("hidden", "");
        return;
      }
      ...
      const thumbLeft =
        maxScrollLeft > 0
          ? Math.round(
              (newGraphContainer.scrollLeft / maxScrollLeft) * maxThumbLeft,
            )
          : 0;
    ...
    if (scrollThumb) {
      ...
      document.addEventListener("pointermove", function (e) {
        ...
        const maxThumbLeft = trackW - thumbW;
        const maxScrollLeft =
          newGraphContainer.scrollWidth - newGraphContainer.clientWidth;
        const deltaScroll = Math.round(dx * (maxScrollLeft / maxThumbLeft));
        newGraphContainer.scrollLeft = Math.min(
          Math.max(0, startScrollLeft + deltaScroll),
          maxScrollLeft,
        );
      });
      ...
      scrollTrack.addEventListener("click", function (e) {
        ...
        const targetLeft = Math.min(
          Math.max(0, clickX - thumbW / 2),
          maxThumbLeft,
        );
        const maxScrollLeft =
          newGraphContainer.scrollWidth - newGraphContainer.clientWidth;
        newGraphContainer.scrollLeft = Math.round(
          (targetLeft / maxThumbLeft) * maxScrollLeft,
        );
      });
    }

    // Use event delegation for better performance with large datasets
    newGraphContainer.addEventListener(
      "mouseenter",
      (e) => {
        if (e.target.tagName === "rect" && e.target.dataset.date) {
          ...
          if (totalPoints === 0) {
            ...
            if (classicSessions > 0)
              tip += `<div>${classicSessions} ${pluralize(classicSessions, "Classic Pomodoro", "Classic Pomodoros")}</div>`;
            if (reverseSessions > 0)
              tip += `<div>${reverseSessions} ${pluralize(reverseSessions, "Reverse Pomodoro", "Reverse Pomodoros")}</div>`;
            if (breakSessions > 0)
              tip += `<div>${breakSessions} ${pluralize(breakSessions, "Break", "Breaks")}</div>`;
          ...
          let headingWidth = 0;
          if (heading) {
            // Temporarily allow the heading to measure naturally
            heading.style.whiteSpace = "nowrap";
            heading.style.display = "inline-block";
            const hRect = heading.getBoundingClientRect();
            headingWidth = Math.ceil(hRect.width);
          }
          // Use heading width + padding as target (limit to available width)
          let tW = Math.min(
            maxAvailableW,
            Math.max(160, headingWidth + 28 || 160),
          );
          tooltip.style.width = tW + "px";

          // If heading alone is wider than available space, allow heading to wrap and use maxAvailableW
          ...
          const desiredX = Math.round(
            relLeft + rectBox.width / 2 + scrollContainer.scrollLeft - tW / 2,
          );
          const minX = scrollContainer.scrollLeft + 8;
          const maxX =
            scrollContainer.scrollLeft + scrollContainer.clientWidth - tW - 8;
          let tooltipX = Math.min(
            Math.max(desiredX, minX),
            Math.max(minX, maxX),
          );
          ...
          if (spaceAbove >= tH + 10) {
            ...
            tooltipY = Math.max(
              ...
              Math.min(
                relTop + scrollContainer.scrollTop - tH - 8,
                scrollContainer.scrollHeight - tH - 8,
              ),
            );
          }
    ...
    if (rangeSelect) {
      ...
      // populate options
      let optionsHtml =
        '<option value="last12">Last 12 Months</option><option value="all">All Time</option>';
      if (years.length) {
        optionsHtml += years
          .map((y) => `<option value="year-${y}">${y}</option>`)
          .join("");
      }
