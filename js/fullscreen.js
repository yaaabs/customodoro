(function () {
  ...
  function updateRadialMenuIcon(isFullscreen) {
    const radialFullscreenBtn = document.getElementById(
      "radial-fullscreen-btn",
    );
    if (!radialFullscreenBtn) return;

    const enterIcon = radialFullscreenBtn.querySelector(
    ...
    if (isFullscreen) {
      if (enterIcon) enterIcon.style.display = "none";
      if (exitIcon) exitIcon.style.display = "block";
      radialFullscreenBtn.querySelector(".radial-tooltip").textContent =
        "Exit Fullscreen";
    } else {
      if (enterIcon) enterIcon.style.display = "block";
      if (exitIcon) exitIcon.style.display = "none";
