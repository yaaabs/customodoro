...
function handleStart() {
  ...
  playSound("start");
  showToast(
    currentMode === "break" ? "Enjoy your break! 😌" : "Time to focus! 💪",
  );
...
function handleStop() {
  ...
  if (currentMode === "reverse") {
    ...
    if (breakLogicMode === "default" && minutes < 5) {
      if (
        confirm(
          "You worked less than 5 minutes. No break earned. Do you want to reset the timer?",
        )
      ) {
...
function switchMode(mode) {
  ...
  if ((isRunning || timerState === "paused") && !isAutoSwitch) {
    ...
    if (isRunning) {
      ...
      message =
        "Timer is paused with unsaved progress. Are you sure you want to switch modes? This will reset your current progress.";
    }
...
document
  .querySelectorAll(".time-btn, .secondary-btn, .tab")
  .forEach((button) => {
...
window.addEventListener("beforeunload", (e) => {
  ...
  if (isRunning || timerState === "paused") {
    ...
    if (isRunning) message = "Timer is still running.";
      ...
      message = "Timer is paused with unsaved progress.";
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (switchBtn) {
    switchBtn.addEventListener("click", async (e) => {
      ...
      const targetMode = switchBtn.textContent.includes("Classic")
        ? "Classic Pomodoro"
        : "Reverse Pomodoro";
      ...
      if (isRunning || timerState === "paused") {
        ...
        if (isRunning) {
          message =
            "Timer is still running. Are you sure you want to switch to a different timer type? This will reset your progress.";
        } else if (timerState === "paused") {
          message =
            "Timer is paused with unsaved progress. Are you sure you want to switch to a different timer type? This will reset your progress.";
        }
        const confirmed = confirm(message);
        if (!confirmed) {
          return;
